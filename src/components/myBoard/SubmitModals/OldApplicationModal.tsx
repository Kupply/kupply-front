...
export default function OldApplicationModal(props: ModalProps) {
  ...
  const [candidateState, setCandidateState] = useState<'default' | 'clicked' | 'inactive'>('default');
  const [passerState, setPasserState] = useState<'default' | 'clicked' | 'inactive'>('default');

  const handleButtonClick = (buttonState: string) => {
    if (buttonState === 'candidate' && candidateState !== 'clicked') {
      setCandidateState('clicked');
      setPasserState('inactive');
    } else if (buttonState === 'passer' && passerState !== 'clicked') {
      setPasserState('clicked');
      setCandidateState('inactive');
    }
  };
