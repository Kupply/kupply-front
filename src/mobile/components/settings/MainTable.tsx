  ...
  return (
    ...
    <Table>
      {rows.slice(0, 4).map((row) => (
        <Row key={row.id} onClick={row.onClick}>
          <Typography size='4.44vw' bold='400' style={{color: 'black'}}>{row.title}</Typography>
          <RightArrow />
        </Row>
      ))}
    </Table>
    <Table>
      <Row key={rows[4].id} onClick={rows[4].onClick}>
        <Typography size='4.44vw' bold='400' style={{color: 'black'}}>{rows[4].title}</Typography>
        <RightArrow />
      </Row>
    </Table>
    </>
...
export const MainTable: React.FC = () => {
  ...
  const rows = [
    ...
    { id: 2, title: '프로필 사진 / 닉네임 변경하기', onClick: () => handleRowClick(2) },
    { id: 3, title: '마이보드 프로필 수정하기', onClick: () => handleRowClick(3) },
    { id: 4, title: '계정관리', onClick: () => handleRowClick(4) },
    { id: 5, title: '약관보기', onClick: () => handleRowClick(5)}
  ];
