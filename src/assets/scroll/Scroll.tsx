...
function ScrollSmall(props: ScrollButtonProps) {
...
function ScrollLarge(props: ScrollButtonProps) {
  const { children = '내용', isChecked = false, ...rest } = props;
  const [scrollActive, setActive] = useState(false);
  const [isHovered, setIsHovered] = useState(false);
...
const ScrollbarSmall = styled.button<{ isChecked: boolean; isHovered: boolean }>`
  ...
  &::-webkit-scrollbar {
    width: 0.3125vw; // (6/1920*100)vw width: 6px;
    height: 100%;
  }
...
const ScrollbarLarge = styled.button<{ isChecked: boolean; isHovered: boolean }>`
  ...
  /* Webkit 기반의 브라우저 Chrome, Safari */
  &::-webkit-scrollbar {
    width: 0.5208vw; // (10/1920*100)vw  width: 10px;
    height: 502px;
  }
  ...
  .scrollbarWrapper {
    ...
    right: 0;
    top: 0;
    width: 0.5283vw; // (10/1920*100)vw  width: 10px;
    height: 100%;
    background: transparent;
    pointer-events: none;
  }
...
export { ScrollSmall, ScrollLarge };
