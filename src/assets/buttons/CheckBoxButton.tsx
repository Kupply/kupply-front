...
const CheckBoxButton01 = (props: CheckBoxButtonProps) => {
  ...
  return <ButtonWrapper01 {...rest}>
    <button onClick={() => onCustomFunction(!isChecked)}>
      <ButtonImage01 isChecked={isChecked}/>
    </button>
    <ButtonText01 style={{fontSize: `${textSize}`, fontWeight: `${textBold}`}}>
      {children}
    </ButtonText01>
    <ArrowImage/>
  </ButtonWrapper01>
...
const ButtonImage01 = ( {isChecked}: {isChecked: boolean}):JSX.Element => {
  ...
  if (isChecked === false) {
    ...
    return (
      <svg style={{fill: 'var(--White, #FFF)'}} xmlns="http://www.w3.org/2000/svg" width="1.458vw" height="28" viewBox="0 0 28 28" fill="none">
...
const ButtonImage02 = ({isChecked} : {isChecked:boolean}):JSX.Element => {
  if (!isChecked) {
    ...
    return (
      <svg style = {{
        fill: 'var(--White, #FFF)'
      }} xmlns="http://www.w3.org/2000/svg" width="0.9375vw" height="18" viewBox="0 0 18 18" fill="none">
